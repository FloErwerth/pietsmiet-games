...
const correctAnswerIn = (roomId: string, player: Player) =>
  socket.emit("correctAnswer/in", roomId, player);
const correctAnswerOut = (listener: (player: Player[]) => void) =>
  socket.on("correctAnswer/out", listener);
const incorrectAnswerIn = (roomId: string, player: Player) =>
...
const questionNextOut = (listener: () => void) =>
  socket.on("question/next/out", listener);
const questionPreviousIn = (roomId: string) =>
  socket.emit("question/previous/in", roomId);
const questionPreviousOut = (listener: () => void) =>
  socket.on("question/previous/out", listener);
